...
OBJ_TEST     := $(patsubst $(SRC_DIR)/%.cpp,$(BUILD_DIR)/hosted/%.o,$(SRC_CPP)) \
                $(patsubst $(TEST_DIR)/%.cpp,$(BUILD_DIR)/hosted/tests/%.o,$(wildcard $(TEST_DIR)/*.cpp))
...
$(TESTBIN): $(OBJ_TEST) | $(BIN_DIR)
	$(ZIGCXX) $(LDFLAGS_HOSTED) -o $@ $(OBJ_TEST) $(LDLIBS_HOSTED)
